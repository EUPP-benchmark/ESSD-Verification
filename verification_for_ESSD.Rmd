...
methods_dict <- c(
  ...
  `MetOffice_IMPROVER-reliabilitycalibration-v1.3.1_v1.3` = "IMPROVER",
...
```{r leadtime-paper, fig.width = 8, fig.height = 4}
scores_mn %>%
  tidyr::pivot_longer(-c(source, step)) %>%
  dplyr::filter(name %in% c("bias", "crps", "s2e", "sd")) %>%
  dplyr::mutate(name = factor(renamer[name], c("CRPS", "Bias", "Ensemble spread", "Spread-error-ratio"))) %>% 
  ggplot(aes(x = step, y = value, col = source)) + 
  geom_hline(
...
```{r rank-histogram-paper, fig.height = 2, fig.width = 10}
ranks %>%
  # dplyr::filter(source != "ECMWF DMO") %>%
...
scores_map %>%
  ...
  geom_point(aes(fill = skill), pch = 21, size = 2, col = 'grey') + 
...
scores_map %>%
  ...
  geom_hline(
    data = gridlines %>% dplyr::mutate(score = renamer[score]) %>% tidyr::drop_na(), 
